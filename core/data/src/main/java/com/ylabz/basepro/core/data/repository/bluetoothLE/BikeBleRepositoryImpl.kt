...
class BikeBleRepositoryImpl @Inject constructor(
    ...
    override fun connectToBike(deviceName: String): Flow<BikeBleRepository.BikeBleData> =
        callbackFlow {
                ...
                device.connectGatt(context, false, object : BluetoothGattCallback() {
                    ...
                    override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
                        if (status == BluetoothGatt.GATT_SUCCESS) {
                            val service =
                                gatt.getService(java.util.UUID.fromString(BIKE_SERVICE_UUID))
                            if (service != null) {
                                val characteristic = service.getCharacteristic(
                                    java.util.UUID.fromString(BIKE_CHAR_UUID)
                                )
                                if (characteristic != null) {
                                    ...
                                    Log.d(
                                        "BikeBleRepository",
                                        "Bike characteristic found, notifications enabled."
                                    )
                                } else {
