...
- Example: `GetCoffeeShopsUseCase` could handle business logic for fetching and filtering coffee
  shops.
...
- **Layered Architecture**: Separates data, domain (optional), and presentation layers for clear
  separation of concerns.
- **Unidirectional Data Flow (UDF)**: Each screen observes a single source of truth for its state,
  making the UI predictable and easier to debug.
- **Modularization**: Core functionalities are isolated in modules, making the app more scalable and
  maintainable.

### Key Patterns and Practices

- **MVVM (Model-View-ViewModel)**: Used for managing UI state in composables.
- **Repository Pattern**: Data access is managed through repositories, providing a single source of
  truth for each feature.
- **Dependency Injection with Hilt**: Each module has its own DI setup to inject dependencies
  efficiently.
...
3. **Configure API Keys**:
    - Add your Yelp API and Google Maps API keys to the appropriate configuration files (e.g.,
      `local.properties` or a dedicated configuration class).
...
- **CoffeeShop Feature**: Navigate to the Coffee Shop screen from the Home screen to find coffee
  shops nearby.
...
Contributions are welcome! Please open an issue or submit a pull request for improvements, bug
